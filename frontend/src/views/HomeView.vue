  ...
  <div class="space-y-6 p-4">
    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      <div v-for="item in filteredItems" :key="item.id" 
        ...
        class="group bg-base-100 border border-base-200 rounded-xl hover:border-primary/40 hover:shadow-lg hover:shadow-primary/5 transition-all duration-300 flex flex-col p-5 cursor-pointer">
        
        <div class="flex items-start gap-4 mb-4">
          <div class="w-12 h-12 rounded-xl flex items-center justify-center shrink-0 transition-colors"
                :class="item.type === 'extension' ? 'bg-primary/10 text-primary group-hover:bg-primary group-hover:text-primary-content' : 'bg-secondary/10 text-secondary group-hover:bg-secondary group-hover:text-secondary-content'">
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="flex items-center gap-2 mt-0.5">
              ...
              <span class="text-xs opacity-40 truncate">@{{ item.author?.username || '匿名' }}</span>
            </div>
          </div>
        </div>
